...
intra_check<-function(chromosome,start,end,plot_type="loops"){
  ...
  chra = chromosome
  sub<-bedpe %>% dplyr::filter(chrom1==chra & chrom2==chra, type %in% c("duplication","inverted-duplication")) %>% dplyr::arrange(start1)
...
#1. For deletion and inversion only 

intra_karo<-function(events){
  ...
  sub<-bedpe %>% dplyr::filter(type %in% events) %>% rowwise() %>% 
    mutate(start=min(start1,start2),end=max(end1,end2)) %>% ungroup()
  ggplot(data=df1,aes(Chrom1,end))+geom_bar(stat="identity",color="black",fill="white",width=0.5)+theme_bw()+coord_flip()+
    ggplot2::geom_segment(data=sub,aes(x=chrom1,xend=chrom1,y=start,yend=end,color=Description),size=6)+
    scale_x_discrete(limits = rev(levels(df1$Chrom1)))+scale_color_brewer(palette = "Paired")+
    facet_wrap(~type)
}

intra_karo(c("deletion","inversion"))

#2. for reciprocal, translocation (inverted) only 
...
#3. for duplication (inverted), inversion, deletion
