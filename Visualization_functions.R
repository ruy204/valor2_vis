...
chromosome_check_reverse<-function(chr,start_position,chunck_length,annotate="population",
                           types=c("inversion","deletion","reciprocal","inverted-reciprocal",
                                   "inverted-duplication","duplication","translocation","inverted-translocation")){
  
  bedpe<-read.delim("C:/PhD/Rotations/Rotation_1/data/SV2/bedpe/combine/jointdf.bedpe",header = F)
  colnames(bedpe)<-c("chrom2","start2","end2","chrom1","start1","end1",
                     "type","score", "samples","Description")
  bedpe[,c(2,3,5,6,8)]<-sapply(bedpe[,c(2,3,5,6,8)],function(x){as.numeric(as.character(x))})
  bedpe$chrom1<-factor(bedpe$chrom1,levels=paste("chr",c(1:22,"X","Y"),sep=""))
  bedpe$chrom2<-factor(bedpe$chrom2,levels=paste("chr",c(1:22,"X","Y"),sep=""))
...
chromosome_check("chr22",43754300,300000,type="inverted-reciprocal")
chromosome_check("chr22",43754300,300000,type="reciprocal")
chromosome_check_reverse("chr4",6340188,500000,type="reciprocal")
chromosome_check_reverse("chr4",6340188,500000,type="inverted-reciprocal")

pdf("C:/PhD/Rotations/Rotation_1/writing_valor2/plots/inversion_hotspot_chr1.pdf",width = 14,height = 7)
p=chromosome_check("chr1",   104100000,100000,type="inversion")
print(p)
dev.off()
